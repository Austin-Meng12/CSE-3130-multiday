...
class Ball(mySprite):
    ...
    def determineBoxDirection(self,POSITION, DIMENSION):
        ...
        if SPRITE_X < self._X + self.getWidth() < SPRITE_X + SPRITE_W + 5  and self._Y + self.getHeight() > SPRITE_Y + SPRITE_H:
           self.setDIRY()   #Bottom
        if SPRITE_X < self._X +  self.getWidth() < SPRITE_X +  SPRITE_W + 5  and self._Y < SPRITE_Y:
           self.setDIRY()       # Top
        if SPRITE_Y < self._Y + self.getHeight() < SPRITE_Y + SPRITE_H + 5 and self._X + self.getWidth() > SPRITE_X + SPRITE_W:
           self.setDIRX()    # Right
        if SPRITE_Y < self._Y + self.getHeight() < SPRITE_Y + SPRITE_H + 5 and self._X  < SPRITE_X:
            self.setDIRX()   # Left
